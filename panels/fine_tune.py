...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        for j, i in enumerate(self.z_deltas):
            ...
            self.labels[f"zdelta{i}"].connect(
                "clicked", self.change_percent_delta, "z_offset", float(i)
            )
        ...
        for j, i in enumerate(self.extrude_deltas):
            ...
            self.labels[f"edelta{i}"].connect(
                "clicked", self.change_percent_delta, "extrude", int(i)
            )
    ...
    def change_babystepping(self, widget, direction):
        if direction == "reset":
            ...
            self._screen._send_action(
                widget, "printer.gcode.script", {"script": "SET_GCODE_OFFSET Z=0 MOVE=1"}
            )
            return
        elif direction == "+":
    ...
    def change_speed(self, widget, direction):
        ...
        self.labels["speedfactor"].set_label(f"  {self.speed:3}%")
        self._screen._send_action(
            widget, "printer.gcode.script", {"script": KlippyGcodes.set_speed_rate(self.speed)}
        )

    def change_percent_delta(self, widget, array, delta):
        logging.info(f"### Delta {delta}")
        if array == "z_offset":
            self.labels[f"zdelta{self.z_delta}"].get_style_context().remove_class(
                "horizontal_togglebuttons_active"
            )
            self.z_delta = delta
        elif array == "speed":
            self.labels[f"sdelta{self.s_delta}"].get_style_context().remove_class(
                "horizontal_togglebuttons_active"
            )
            self.s_delta = delta
        elif array == "extrude":
            self.labels[f"edelta{self.e_delta}"].get_style_context().remove_class(
                "horizontal_togglebuttons_active"
            )
            self.e_delta = delta
